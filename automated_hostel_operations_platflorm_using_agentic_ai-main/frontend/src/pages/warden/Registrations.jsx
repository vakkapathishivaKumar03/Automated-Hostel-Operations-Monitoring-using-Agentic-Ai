...
const Registrations = () => {
  ...
  const [loadingToast, setLoadingToast] = useState({ open: false, message: '' });
  ...
  const handleApproveConfirm = async () => {
    ...
    try {
      ...
      setLoadingToast({ open: false, message: '' });
      if (data.success) {
    ...
    <div className="admin-registrations-page">
      <div className="page-header page-header-card">
        <div className="header-content page-header-text">
          <h1 className="page-title">Warden Registration Review</h1>
      ...
      {loadingToast.open && (
        <div className="toast toast-loading">
          ...
          <span>{loadingToast.message}</span>
        </div>
      )}
    </div>
  );
